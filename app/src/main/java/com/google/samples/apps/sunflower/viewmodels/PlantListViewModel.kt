...
class PlantListViewModel @Inject internal constructor(
    ...
    private val growZone: MutableStateFlow<Int> = MutableStateFlow(
        savedStateHandle.get(GROW_ZONE_SAVED_STATE_KEY) ?: NO_GROW_ZONE // 默认：-1全部展示
    )
//    监听 growZone 发生改变,
//    flatMapLatest适用于避免重复操作，只需对最新的一次操作进行处理的情况。
//    如网络请求。输入用户名，直到用户输入稳定，只需要取稳定后最新的。
//    通过flatMapLatest方法取最新的一次输入，发起网络请求，服务器响应后判断用户输入的用户名字符串是否合法。

    val plants: LiveData<List<Plant>> = growZone.flatMapLatest { zone ->
        if (zone == NO_GROW_ZONE) { // 区域 = -1
            plantRepository.getPlants() // 返回所有植被
        } else {
            plantRepository.getPlantsWithGrowZoneNumber(zone) // 返回指定区域的植被
        }
    }.asLiveData()
